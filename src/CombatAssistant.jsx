...
const enemiesList = {
  ...
  fanatic: {
    ...
    maxArmor: 5,
    moves: {
      sword: { damage: 7, armor: 6, uses: 3 },
      shield: { damage: 5, armor: 6, uses: 3 },
      spell: { damage: 5, armor: 6, uses: 3 }
    }
  },
  forestRobe: {
    name: "Forest Robe", 
    ...
    maxArmor: 5,
    moves: {
      sword: { damage: 5, armor: 4, uses: 3 },
      shield: { damage: 4, armor: 5, uses: 3 },
      spell: { damage: 10, armor: 5, uses: 3 }
    }
  }
}
...
export default function CombatAssistant() {
  ...
  async function savePlayer(name) {
    try {
      ...
      const playerData = {
        ...
        max_hp: player.maxHp,
        armor: player.armor,
        max_armor: player.maxArmor,
        moves: player.moves,
        uses: player.uses,
        cooldowns: player.cooldowns,
      };

      // Vérifie si player existe déjà
      ...
      if (selectError) {
        ...
        alert('Error fetching existing player');
        return;
      }
  ...
  async function loadPlayer() {
    try {
      ...
      // Recharge dans le state React
      setPlayer({
        hp: data.hp,
        maxHp: data.max_hp,
        armor: data.armor,
        maxArmor: data.max_armor,
        moves: data.moves,
        uses: data.uses,
    ...
    <Container>
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={4}>
          ...
          <Button variant="contained" color="secondary" sx={{ ml: 2 }} onClick={() => savePlayer()}>Save Player</Button>
        </Grid>
